"""Guatemala SIB banking supervisor source.

Queries Guatemala SIB (Superintendencia de Bancos) for supervised entities.
...
SIB_URL = "https://www.sib.gob.gt/"


@register
class GtSibSource(BaseSource):
    """Query Guatemala SIB supervised banks and insurance entities."""
    ...
    def meta(self) -> SourceMeta:
        return SourceMeta(
            name="gt.sib",
            display_name="SIB — Entidades Supervisadas (GT)",
            description="Guatemala SIB: supervised banks and insurance entities by name",
            country="GT",
            url=SIB_URL,
    ...
    def query(self, input: QueryInput) -> BaseModel:
        ...
        if not search_term:
            raise SourceError("gt.sib", "Entity name is required")
        return self._query(search_term.strip(), audit=input.audit)

    def _query(self, search_term: str, audit: bool = False) -> GtSibResult:
        ...
        if audit:
            ...
            collector = AuditCollector("gt.sib", "entity_name", search_term)

        with browser.page(SIB_URL) as page:
            try:
                if collector:
                ...
                if not search_input:
                    raise SourceError("gt.sib", "Could not find search input field")
                ...
                raise SourceError("gt.sib", f"Query failed: {e}") from e

    def _parse_result(self, page, search_term: str) -> GtSibResult:
        from datetime import datetime

        body_text = page.inner_text("body")
        result = GtSibResult(queried_at=datetime.now(), search_term=search_term)
