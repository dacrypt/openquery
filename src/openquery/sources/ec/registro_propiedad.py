"""Registro Propiedad source — Ecuadorian property registry.

Queries the Registro de la Propiedad de Ecuador for ownership and lien data.

Flow:
1. Navigate to Ecuador property registry consultation page
...
REGISTRO_EC_URL = "https://www.registrodelapropiedad.gob.ec/consulta"


@register
class RegistroPropiedadSource(BaseSource):
    """Query Ecuadorian property registry."""
    ...
    def meta(self) -> SourceMeta:
        return SourceMeta(
            name="ec.registro_propiedad",
            display_name="Registro de la Propiedad Ecuador",
            description="Ecuadorian property registry — ownership and liens",
            country="EC",
            url=REGISTRO_EC_URL,
    ...
    def query(self, input: QueryInput) -> BaseModel:
        ...
        if not search_value:
            raise SourceError("ec.registro_propiedad", "property_code is required")
    ...
    def _query(self, search_value: str, audit: bool = False) -> RegistroPropiedadResult:
        ...
        if audit:
            ...
            collector = AuditCollector("ec.registro_propiedad", "custom", search_value)

        with browser.page(REGISTRO_EC_URL) as page:
            try:
                if collector:
                ...
                search_input = page.query_selector(
                    'input[type="text"], input[id*="clave"], '
                    'input[id*="codigo"], input[id*="predio"]'
                )
                if not search_input:
                    raise SourceError("ec.registro_propiedad", "Could not find property code input")
                ...
                raise SourceError("ec.registro_propiedad", f"Query failed: {e}") from e
    ...
    def _parse_result(self, page, search_value: str) -> RegistroPropiedadResult:
        ...
        for line in body_text.split("\n"):
            ...
            elif "gravamen" in lower or "hipoteca" in lower or "prohibicion" in lower:
                liens.append(stripped)

        return RegistroPropiedadResult(
