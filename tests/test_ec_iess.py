"""Unit tests for Ecuador IESS social security affiliation source."""

from __future__ import annotations

from unittest.mock import MagicMock

from openquery.models.ec.iess import IessResult
from openquery.sources.ec.iess import IessSource


class TestIessResult:
    """Test IessResult model."""

    def test_default_values(self):
        data = IessResult()
        assert data.cedula == ""
        assert data.affiliation_status == ""
        assert data.employer == ""
        assert data.details == {}
        assert data.audit is None

    def test_round_trip_json(self):
        data = IessResult(
            cedula="1234567890",
            affiliation_status="AFILIADO ACTIVO",
            employer="EMPRESA XYZ S.A.",
            details={"Estado": "ACTIVO"},
        )
        json_str = data.model_dump_json()
        restored = IessResult.model_validate_json(json_str)
        assert restored.cedula == "1234567890"
        assert restored.affiliation_status == "AFILIADO ACTIVO"
        assert restored.employer == "EMPRESA XYZ S.A."

    def test_audit_excluded_from_json(self):
        data = IessResult(cedula="1234567890", audit={"evidence": "pdf"})
        json_str = data.model_dump_json()
        assert "audit" not in json_str
        assert data.audit == {"evidence": "pdf"}


class TestIessSourceMeta:
    """Test IessSource metadata."""

    def test_meta_name(self):
        source = IessSource()
        assert source.meta().name == "ec.iess"

    def test_meta_country(self):
        source = IessSource()
        assert source.meta().country == "EC"

    def test_meta_requires_browser(self):
        source = IessSource()
        assert source.meta().requires_browser is True

    def test_meta_requires_captcha(self):
        source = IessSource()
        assert source.meta().requires_captcha is False

    def test_meta_rate_limit(self):
        source = IessSource()
        assert source.meta().rate_limit_rpm == 10

    def test_default_timeout(self):
        source = IessSource()
        assert source._timeout == 30.0

    def test_custom_timeout(self):
        source = IessSource(timeout=60.0)
        assert source._timeout == 60.0

    def test_meta_supported_inputs(self):
        from openquery.sources.base import DocumentType

        source = IessSource()
        assert DocumentType.CEDULA in source.meta().supported_inputs


class TestParseResult:
    """Test _parse_result parsing logic with mocked page."""

    def _make_page(self, body_text: str, table_rows: list[tuple] | None = None) -> MagicMock:
        mock_page = MagicMock()
        mock_page.inner_text.return_value = body_text

        if table_rows:
            mock_rows = []
            for label, value in table_rows:
                mock_row = MagicMock()
                mock_cells = []
                for text in (label, value):
                    mock_cell = MagicMock()
                    mock_cell.inner_text.return_value = text
                    mock_cells.append(mock_cell)
                mock_row.query_selector_all.return_value = mock_cells
                mock_rows.append(mock_row)
            mock_page.query_selector_all.return_value = mock_rows
        else:
            mock_page.query_selector_all.return_value = []

        return mock_page

    def test_parse_cedula_preserved(self):
        source = IessSource()
        page = self._make_page("Sin resultado")
        result = source._parse_result(page, "1234567890")
        assert result.cedula == "1234567890"

    def test_parse_from_body_text(self):
        source = IessSource()
        page = self._make_page(
            "Estado de Afiliacion: AFILIADO ACTIVO\nEmpresaEmpleador: EMPRESA ABC\n"
        )
        result = source._parse_result(page, "1234567890")
        assert result.affiliation_status == "AFILIADO ACTIVO"
        assert result.employer == "EMPRESA ABC"

    def test_parse_from_table(self):
        source = IessSource()
        page = self._make_page(
            "Resultado",
            table_rows=[
                ("Estado Afiliacion", "VOLUNTARIO"),
                ("Empleador", "EMPRESA DEF S.A."),
            ],
        )
        result = source._parse_result(page, "0987654321")
        assert result.affiliation_status == "VOLUNTARIO"
        assert result.employer == "EMPRESA DEF S.A."
        assert result.details["Empleador"] == "EMPRESA DEF S.A."

    def test_parse_empty_body(self):
        source = IessSource()
        page = self._make_page("")
        result = source._parse_result(page, "1234567890")
        assert result.cedula == "1234567890"
        assert result.affiliation_status == ""
