...
KNOWN_STATUS: dict[str, str] = {
    # WAF/GeoIP blocked — requires Colombian IP or residential proxy
    "co.estado_cedula": "WAF_BLOCKED: Registraduría blocks non-Colombian IPs",
    "co.nombre_completo": "WAF_BLOCKED: Registraduría blocks non-Colombian IPs",
    "co.defuncion": "WAF_BLOCKED: Registraduría blocks non-Colombian IPs",
    "co.puesto_votacion": "WAF_BLOCKED: Registraduría blocks non-Colombian IPs",
    "co.estado_tramite_cedula": "WAF_BLOCKED: Registraduría blocks non-Colombian IPs",
    "co.registro_civil": "WAF_BLOCKED: Registraduría blocks non-Colombian IPs",
    "co.estado_cedula_extranjeria": "WAF_BLOCKED: Migración blocks non-Colombian IPs",
    "co.libreta_militar": "WAF_BLOCKED: mil.co blocks headless browsers",
    "co.colpensiones": "WAF_BLOCKED: Colpensiones blocks headless browsers (Attack ID 20000051)",
    "co.rethus": "WAF_BLOCKED: SISPRO/ReTHUS blocks headless browsers",
    "co.supersociedades": "WAF_BLOCKED: Supersociedades blocks headless browsers",
    "co.adres": "SITE_MAINTENANCE: ADRES currently in maintenance mode",
    # Auth required — needs credentials we don't have
    "co.rne": "AUTH_REQUIRED: Needs CRC portal credentials (usuario/password)",
    "co.fasecolda": "AUTH_REQUIRED: Needs manual bearer token capture from authenticated session",
    "ec.sri_ruc": "AUTH_REQUIRED: SRI API requires authorization token",
    # Site genuinely down
    "co.inpec": "SITE_DOWN: INPEC returns 504 Gateway Timeout (Azure)",
    "co.certificado_tradicion": "SITE_DOWN: supernotariado.gov.co unreachable (30s timeout)",
    "ec.senescyt": "SITE_DOWN: senescyt.gob.ec unreachable (30s timeout)",
    "pe.servir_sanciones": "SITE_DOWN: sanciones.gob.pe unreachable (30s timeout)",
    "cl.fiscalizacion": "SITE_DOWN: Site unreachable (30s timeout)",
    "mx.repuve": "SITE_DOWN: repuve.gob.mx:8443 unreachable (30s timeout)",
    # reCAPTCHA blocking
    "co.contraloria": "RECAPTCHA: cfiscal.contraloria.gov.co has reCAPTCHA v2",
    "co.rues": "RECAPTCHA: rues.org.co has reCAPTCHA v2 on search",
    "co.sisben": "RECAPTCHA: reportes.sisben.gov.co has invisible reCAPTCHA v2",
    # No search form exists
    "co.jep": "NO_FORM: JEP is SharePoint — no direct person/document search form",
    # RUNT family — API changed to require Azure B2C authentication
    "co.runt": "AUTH_CHANGED: RUNT API now requires Azure B2C authenticated session",
    "co.runt_conductor": "AUTH_CHANGED: RUNT API now requires Azure B2C authenticated session",
    "co.runt_soat": "AUTH_CHANGED: RUNT API now requires Azure B2C authenticated session",
    "co.runt_rtm": "AUTH_CHANGED: RUNT API now requires Azure B2C authenticated session",
    # Sites that block from non-Colombian IPs (selector timeouts)
    "co.fopep": "WAF_BLOCKED: FOPEP blocks headless browsers",
    "co.soi": "WAF_BLOCKED: SOI blocks headless browsers",
    "co.seguridad_social": "WAF_BLOCKED: miseguridadsocial.gov.co blocks headless browsers",
    "co.ruaf": "WAF_BLOCKED: SISPRO/RUAF blocks headless browsers",
    "co.snr": "WAF_BLOCKED: SNR blocks headless browsers",
    "co.mi_casa_ya": "WAF_BLOCKED: Mi Casa Ya portal blocks headless browsers",
    "co.copnia": "WAF_BLOCKED: COPNIA blocks headless browsers",
    "co.garantias_mobiliarias": "WAF_BLOCKED: Garantias Mobiliarias blocks headless browsers",
    "co.tutelas": "WAF_BLOCKED: Rama Judicial blocks headless browsers",
    "co.consulta_procesos": "WAF_BLOCKED: Rama Judicial consultaprocesos blocks headless browsers",
    "co.retencion_vehiculos": "WAF_BLOCKED: Barranquilla portal blocks headless browsers",
    # Socrata API datasets removed or moved
    ...
    # Sites down for non-CO visitors
    "pe.sunat_ruc": "SITE_DOWN: e-consultaruc.sunat.gob.pe connection reset from non-PE IPs",
    # Timeout from outside the country
    "mx.curp": "GEOBLOCKED: gob.mx API times out from non-Mexican IPs",
    "mx.sat_efos": "GEOBLOCKED: SAT portal blocks non-Mexican IPs",
    "mx.siem": "GEOBLOCKED: SIEM portal blocks non-Mexican IPs",
    "ar.afip_cuit": "GEOBLOCKED: AFIP/ARCA blocks non-Argentine IPs",
    "ar.dnrpa": "GEOBLOCKED: DNRPA blocks non-Argentine IPs",
    "cl.sii_rut": "GEOBLOCKED: SII blocks or CAPTCHAs non-Chilean IPs",
    "cl.pjud": "GEOBLOCKED: PJUD blocks non-Chilean IPs",
    "pe.poder_judicial": "GEOBLOCKED: PJ blocks non-Peruvian IPs",
    "pe.osce_sancionados": "GEOBLOCKED: OSCE blocks non-Peruvian IPs",
    "ec.supercias": "GEOBLOCKED: Supercias blocks non-Ecuadorian IPs",
    "ec.ant_citaciones": "API_ERROR: ANT API returns HTTP 500 (server-side error)",
    "co.validar_policia": "WAF_BLOCKED: Policia portal requires ASP.NET postback timing (unreliable from headless)",
    "co.rnmc": "WAF_BLOCKED: Policia portal requires ASP.NET postback timing (unreliable from headless)",
}
